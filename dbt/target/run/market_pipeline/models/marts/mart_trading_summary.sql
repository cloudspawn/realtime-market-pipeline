    ...
    create or replace table `bigquery-486314`.`market_data`.`mart_trading_summary`
...
with trades as (
    select * from `bigquery-486314`.`market_data`.`int_trades_aggregated`
),

latest_prices as (
    select * from `bigquery-486314`.`market_data`.`int_prices_latest`
),
